jQuery(document).ready(function ($) {
  ...
  var header = $('.header');
	...
	//thêm giỏ hàng
	  ...
	  if (!selectedSize) {
		  alert('Hãy chọn dung tích trước khi thêm sản phẩm vào giỏ hàng');
	  } else {
  ...
  //Nhảy giá khi chọn dung tích
  let sizes = document.querySelectorAll('.size span');
  let productPrice = document.querySelector('.product_price');
  let originalPrice = document.querySelector('.original_price');
  
  sizes.forEach(function(size) {
	  size.addEventListener('click', function() {
		  // Xóa class 'selected' khỏi tất cả các phần tử 'span'
		  sizes.forEach(function(size) {
			  size.classList.remove('selected');
		  });
		  
		  // Thêm class 'selected' vào phần tử 'span' được chọn
		  this.classList.add('selected');
		  
		  // Cập nhật giá tương ứng với dung tích được chọn
		  if (this.textContent === '40ML') {
						  currentProduct = 'Kem dưỡng ẩm - 40ML';
						  currentProductPrice = '369.000';
						  productPrice.textContent = '399.000';
			  originalPrice.textContent = '369.000';
		  } else if (this.textContent === '100ML') {
						  currentProduct = 'Kem dưỡng ẩm - 100ML';
						  currentProductPrice = '569.000';
			  productPrice.textContent = '569.000';
						  originalPrice.textContent = '635.000';
		  }
	  });
  });
